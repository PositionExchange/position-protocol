...
abstract contract LimitOrderManager is PositionHouseStorage, Base {
    ...
    function _internalCancelAllPendingOrder(
        ...
        PositionLimitOrder.Data[] memory _reduceOrders = reduceLimitOrders[_pmAddress][_trader];
        uint256 totalRefundMargin;
        if (_increaseOrders.length != 0) {
            totalRefundMargin = PositionHouseFunction.getTotalPendingLimitOrderMargin(_positionManager, _increaseOrders, _reduceOrders);
        }
